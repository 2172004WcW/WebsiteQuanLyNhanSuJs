...
## 📝 Ví Dụ Test File (Template)

### File: `frontend/src/pages/__tests__/login.test.js` (Người 1 - UC #1)

```javascript
/**
 * Unit Test: UC #1 - Đăng nhập/Đăng xuất
 * Người phụ trách: [Tên]
 * Source files: Login.jsx, authService.js
 */
...
describe('UC #1: Đăng nhập/Đăng xuất', () => {
  ...
  describe('Backend - authService.login()', () => {
    test('returns token and role for valid credentials', async () => {
      // Mock database
    ...
    test('throws AUTH error for invalid credentials', async () => {
      await expect(login('admin', 'wrongpass')).rejects.toThrow('AUTH');
    });

    test('throws AUTH error for disabled account', async () => {
      await expect(login('disabled_user', 'password')).rejects.toThrow('AUTH');
    });
  });
});
```
